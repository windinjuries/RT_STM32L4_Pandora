...
static int ap3216c_write_reg(unsigned char reg, unsigned char cmd)
{
    ...
    ret = st_i2c_transmit(AP3216C_I2C, AP3216C_ADDR, buf, 2, ST_I2C_OPT_START_NOEND);
    if (ret < 0) {
...
static int ap3216c_read_reg( unsigned char reg, unsigned char *val)
{
    ...
    ret = st_i2c_transmit(AP3216C_I2C, AP3216C_ADDR, buf, 1, ST_I2C_OPT_START_END); //write register address to the device
    if( ret < 0 )
    {
        rt_kprintf("write cmd to ap3216c register failure.\n");
        return -1;
